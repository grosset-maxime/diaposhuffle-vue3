...
const openedPanels = ref(panels
  .filter((panel) => panel.opened)
  .map((panel) => panel.id))
</script>

<template>
  <v-card class="help-diaposhuffle-ctn">
    <v-expansion-panels multiple focusable v-model="openedPanels">
      <v-expansion-panel
        ...
        bg-color="background"
      >
        <v-expansion-panel-title color="primary">{{ panel.title }}</v-expansion-panel-title>
